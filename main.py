...
def emit_views(la: list[str]) -> None:
    ...
    for n in range(18):
        ...
        w(la, "        bps = p.rewardBpsAnnual;")
        w(la, "        phaseRaw = uint8(p.phase);")
        w(la, f"        staked = staked ^ uint256(_SALT_{n % len(HEX32)}) & 0; // noop bind for compiler")
        w(la, "    }")
        w(la, "")
    w(la, "    function epochMix(uint256 epochId) external view returns (bytes32 hA, bytes32 hB, uint256 weight) {")
    w(la, f"        if (epochId == 0 || epochId > {EPOCH_SLOTS}) revert VS_BadEpoch();")
    w(la, "        VoloEpochCell storage e = epochs[epochId];")
    w(la, "        return (e.mixHA, e.mixHB, e.weightSum);")
    w(la, "    }")
    w(la, "")
    w(la, "    function pendingRewards(uint256 poolId, address staker) external view returns (uint256) {")
    w(la, "        return _pendingReward(poolId, staker);")
    w(la, "    }")
    w(la, "")
    w(la, "    function poolMeta(uint256 poolId)")
    w(la, "        external")
    w(la, "        view")
    w(la, "        returns (")
    w(la, "            VoloAssetKind assetKind,")
    w(la, "            VoloPoolPhase phase,")
    w(la, "            uint64 lockSeconds,")
    w(la, "            uint256 rewardBps,")
    w(la, "            uint256 minDeposit,")
    w(la, "            uint256 capWei,")
    w(la, "            uint256 totalStaked")
    w(la, "        )")
    w(la, "    {")
    w(la, "        VoloPoolLine storage p = pools[poolId];")
    w(la, "        return (")
    ...
    w(la, "            p.phase,")
    w(la, "            p.lockSeconds,")
    w(la, "            p.rewardBpsAnnual,")
    w(la, "            p.minDepositWei,")
    w(la, "            p.capWei,")
    w(la, "            p.totalStakedWei")
    w(la, "        );")
    w(la, "    }")
    w(la, "")
    w(la, "    function anchorEquals(address candidate, uint8 slot) external view returns (bool) {")
    ...
    w(la, "        revert VS_BadEpoch();")
    w(la, "    }")
    w(la, "")
    w(la, "    function contractBalanceNative() external view returns (uint256) {")
